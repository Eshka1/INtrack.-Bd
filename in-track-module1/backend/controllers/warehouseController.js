...
const createWarehouse = asyncHandler(async (req, res) => {
  ...
  // Subscription tier gatekeeping preview (same pattern as AssetCategory's
  // maxMaterialTypes check) -- only applies to new top-level sites.
  if (!parent) {
    const tenant = await Tenant.findById(req.tenant._id).populate('subscriptionTier');
    if (tenant.subscriptionTier) {
      const topLevelCount = await Warehouse.countDocuments(
        req.scoped({ isActive: true, parentLocation: null })
      );
      if (topLevelCount >= tenant.subscriptionTier.maxWarehouses) {
        throw new AppError(
          `Your ${tenant.subscriptionTier.name} plan allows a maximum of ${tenant.subscriptionTier.maxWarehouses} warehouse location(s). Upgrade to add more.`,
          403
        );
      }
    }
