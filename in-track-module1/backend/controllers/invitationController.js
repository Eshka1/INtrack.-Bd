...
const createInvitation = asyncHandler(async (req, res) => {
  ...
  // Enforce subscription seat limit before creating the invite
  const tenant = await Tenant.findById(req.tenant._id).populate('subscriptionTier');
  const activeUserCount = await User.countDocuments({ tenantId: req.tenantId, deletedAt: null, isActive: true });
  const pendingInviteCount = await Invitation.countDocuments(req.scoped({ status: 'pending' }));

  if (tenant.subscriptionTier && activeUserCount + pendingInviteCount >= tenant.subscriptionTier.maxUsers) {
    throw new AppError(
      `Your ${tenant.subscriptionTier.name} plan allows a maximum of ${tenant.subscriptionTier.maxUsers} users. Upgrade to invite more.`,
      403
    );
  }
