...
// CORS configuration (supports React/Vite dev ports and custom CLIENT_URL)
const allowedOrigins = [
  process.env.CLIENT_URL,
  'http://localhost:5173', // Vite default port
  'http://localhost:3000'  // Standard CRA/Next dev port
].filter(Boolean);
...
app.use(cors({
  origin: (origin, callback) => {
    ...
    return callback(null, true); // Permissive in dev to prevent CORS blocks during multi-module merges
  },
  credentials: true
}));
...
// Module 2 (Inventory, Suppliers, Recipes, PO Ingestion, Manufacturing Runs)
app.use('/api', module2Routes);
